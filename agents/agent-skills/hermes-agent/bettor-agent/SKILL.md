...
compatibility: Requires the Agentic Prediction Market app running (APM_API_URL) and the dreamdex-event-contracts MCP server (proxy-servers/shared/mcpServer.ts) registered under mcp_servers in Hermes' config.yaml — see references/setup.md.
allowed-tools: mcp_dreamdex_event_contracts_list_markets mcp_dreamdex_event_contracts_get_market mcp_dreamdex_event_contracts_get_order_book mcp_dreamdex_event_contracts_place_order mcp_dreamdex_event_contracts_mint_set mcp_dreamdex_event_contracts_get_positions mcp_dreamdex_event_contracts_redeem mcp_dreamdex_event_contracts_faucet mcp_dreamdex_event_contracts_get_erc8004_status mcp_dreamdex_event_contracts_register_erc8004_identity
metadata:
  hermes:
...
server's side (`AGENT_WALLET_PRIVATE_KEY`, resolved from Hermes' own secrets — see
...
tsx agents/agent-skills/hermes-agent/cli/index.ts place_order --agent <your-agent-id> \
