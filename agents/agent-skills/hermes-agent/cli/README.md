...
the result to the app's activity feed, without Hermes' own reasoning core ever deciding *which*
tool to call. Same engine as the internal `demo-agents/cli/` (Ada/Nomi/Sage) — see
...
Hermes' own model pick a `dreamdex-event-contracts` MCP tool from a plain-English ask — flexible,
but every call costs a reasoning turn. For a **deterministic** request — the kind a messenger
gateway or a fixed Open WebUI action can already recognize word-for-word — that reasoning is pure
overhead: tokens spent (via OpenRouter or whatever backs Hermes' model) on a decision that was
never actually in question, and a dependency on that model staying up and fast. This CLI skips it.

```mermaid
flowchart LR
    subgraph Fixed["Recognized phrasing — cheap path"]
        M1(["Telegram/WhatsApp message,\nOpen WebUI action, or a\nfixed terminal command"]) --> Gateway["Your gateway/router\nmatches it to a known command\n(no model call)"]
        Gateway --> CLI["this CLI\ntsx .../cli/index.ts <tool> --agent <id> [flags]"]
    end
    subgraph OpenEnded["Open-ended ask — full reasoning path"]
        M2(["\"Find me a good trade\nright now\""]) --> Hermes["Hermes' reasoning core\n(bettor-agent / creator-agent skill)"]
        Hermes --> MCP["dreamdex-event-contracts\nMCP tool call"]
    end
...
| Where | How |
|---|---|
| Hermes' `terminal` / `execute_code` sandbox | Point a command or a `hermes.config.yaml` skill `config` entry at this script's path; Hermes passes declared env vars through to that sandbox automatically |
| A messenger gateway (Telegram, WhatsApp, etc.) in front of Hermes | Match the incoming text against the phrasings in the table below (or your own patterns), then shell out to the matching CLI command and reply with its output — no model call in between |
| Open WebUI | Wire a fixed action/button to a specific CLI invocation for one-click deterministic actions (e.g. "Register my identity") |

Exact Hermes config syntax is fast-moving — check
[hermes-agent.nousresearch.com/docs](https://hermes-agent.nousresearch.com/docs) for your installed
version.
...
```sh
# From the repo root:
tsx agents/agent-skills/hermes-agent/cli/index.ts <command> --agent <agentId> [--<key> <value> ...]

# The flagship case — check status, then register an identity, no LLM, no tokens spent:
tsx agents/agent-skills/hermes-agent/cli/index.ts status --agent hermes-bettor
...
tsx agents/agent-skills/hermes-agent/cli/index.ts list
```
...
(`...` stands in for `tsx agents/agent-skills/hermes-agent/cli/index.ts` in every row above.)
...
[../README.md](../README.md) (Hermes Agent skills overview) ·
[../../open-claw/cli/README.md](../../open-claw/cli/README.md) (same CLI, OpenClaw) ·
