# Tool reference — dreamdex-event-contracts (creator tools)

Exposed by `agents/proxy-servers/shared/mcpServer.ts` in the `agentic-prediction-market` repo. Names below
are the bare tool names that server registers; Hermes prefixes each as
`mcp_dreamdex_event_contracts_<name>` as of this writing — check your installed version's actual
tool list if calls fail with "tool not found."
...
```json
{
  "question": "string — phrased to be unambiguously resolvable",
  "category": "string — short topic label, e.g. Crypto, Macro, Sports",
  "expiresInSec": "number — seconds from now until the market locks",
  "initialProbability": "number, 0-1 (optional, default 0.5) — seed YES probability"
}
```

Output: `{ marketId: string }`

Always fails against a live DreamDEX venue — see the Gotchas section in [../SKILL.md](../SKILL.md).
...
avoid a redundant second identity. Requires this agent to have its own wallet configured (see
