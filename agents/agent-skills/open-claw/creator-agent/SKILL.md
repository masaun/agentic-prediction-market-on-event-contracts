...
compatibility: Requires the Agentic Prediction Market app running (APM_API_URL) and the dreamdex-event-contracts MCP server (proxy-servers/shared/mcpServer.ts) registered under mcp.servers in OpenClaw's config — see references/setup.md. Market creation only succeeds against the app's mock engine (MARKET_ENGINE=mock); it always fails against a live DreamDEX venue by design.
allowed-tools: dreamdex-event-contracts__list_markets dreamdex-event-contracts__get_market dreamdex-event-contracts__get_order_book dreamdex-event-contracts__create_market dreamdex-event-contracts__faucet dreamdex-event-contracts__get_erc8004_status dreamdex-event-contracts__register_erc8004_identity
metadata:
  openclaw:
...
tsx agents/agent-skills/open-claw/cli/index.ts create_market --agent <your-agent-id> \
